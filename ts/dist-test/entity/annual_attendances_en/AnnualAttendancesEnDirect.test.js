...
(0, node_test_1.describe)('AnnualAttendancesEnDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.ChineseMedicineClinicsSDK({
    ...
    (0, node_test_1.test)('direct-list-annual_attendances_en', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-annual_attendances_en', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        const result = await client.direct({
            path: 'cmctr/annual-attendances-en.json',
            method: 'GET',
            params,
            query,
        });
...
function liveScenariosActive() { return false && process.env.CHINESE_MEDICINE_CLINICS_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.ChineseMedicineClinicsSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['CHINESE_MEDICINE_CLINICS_TEST_ANNUAL_ATTENDANCES_EN_ENTID'];
    ...
    const client = new __1.ChineseMedicineClinicsSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=AnnualAttendancesEnDirect.test.js.map