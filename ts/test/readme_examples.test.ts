...
//     (\`new ChineseMedicineClinicsSDK(...)\` / \`ChineseMedicineClinicsSDK.test(...)\` become
//     \`ChineseMedicineClinicsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ChineseMedicineClinicsSDK'
...
const TEST_SEED = {"entity":{"annual_attendances_en":{"test01":{"id":"test01"}},"annual_attendances_sc":{"test01":{"id":"test01"}},"annual_attendances_tc":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new ChineseMedicineClinicsSDK(...)\` and any
// \`ChineseMedicineClinicsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ChineseMedicineClinicsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ChineseMedicineClinicsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
