...
describe('AnnualAttendancesScDirect', async () => {
  ...
  test('direct-list-annual_attendances_sc', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-annual_attendances_sc', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CHINESE_MEDICINE_CLINICS_TEST_ANNUAL_ATTENDANCES_SC_ENTID']
