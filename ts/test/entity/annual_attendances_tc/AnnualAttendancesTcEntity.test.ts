...
describe('AnnualAttendancesTcEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'annual_attendances_tc.' + op, live)) return
    }
    ...
    let annual_attendances_tc_ref01_data = Object.values(setup.data.existing.annual_attendances_tc)[0] as any

    // LIST
    const annual_attendances_tc_ref01_ent = client.AnnualAttendancesTc()
    const annual_attendances_tc_ref01_match: any = {}

    const annual_attendances_tc_ref01_list = (await annual_attendances_tc_ref01_ent.list(annual_attendances_tc_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/annual_attendances_tc/AnnualAttendancesTcTestData.json')
  ...
  let idmap = transform(
    ['annual_attendances_tc01','annual_attendances_tc02','annual_attendances_tc03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
