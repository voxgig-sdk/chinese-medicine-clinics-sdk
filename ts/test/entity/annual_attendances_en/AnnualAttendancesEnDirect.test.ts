...
describe('AnnualAttendancesEnDirect', async () => {
  ...
  test('direct-list-annual_attendances_en', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-annual_attendances_en', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CHINESE_MEDICINE_CLINICS_TEST_ANNUAL_ATTENDANCES_EN_ENTID']
