...
describe('AnnualAttendancesEnEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'annual_attendances_en.' + op, live)) return
    }
    ...
    let annual_attendances_en_ref01_data = Object.values(setup.data.existing.annual_attendances_en)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/annual_attendances_en/AnnualAttendancesEnTestData.json')
