...
require("chinese-medicine-clinics_types")
...
local ChineseMedicineClinicsSDK = {}
ChineseMedicineClinicsSDK.__index = ChineseMedicineClinicsSDK
...
ChineseMedicineClinicsSDK._make_feature = _make_feature


function ChineseMedicineClinicsSDK.new(options)
  local self = setmetatable({}, ChineseMedicineClinicsSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function ChineseMedicineClinicsSDK:options_map()
...
function ChineseMedicineClinicsSDK:get_utility()
  return Utility.copy(self._utility)
end


function ChineseMedicineClinicsSDK:get_root_ctx()
  return self._rootctx
end


function ChineseMedicineClinicsSDK:prepare(fetchargs)
...
function ChineseMedicineClinicsSDK:direct(fetchargs)
...
function ChineseMedicineClinicsSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function ChineseMedicineClinicsSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "ChineseMedicineClinicsSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function ChineseMedicineClinicsSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function ChineseMedicineClinicsSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "ChineseMedicineClinicsSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function ChineseMedicineClinicsSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = ChineseMedicineClinicsSDK.new(sdkopts)
  sdk.mode = "test"
...
return ChineseMedicineClinicsSDK
