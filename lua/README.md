...
The Lua SDK for the ChineseMedicineClinics API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:AnnualAttendancesEn(nil):list(nil, nil)
...
local result, err = client:ChineseMedicineClinics(nil):load(
  { id = "test01" }, nil
)
