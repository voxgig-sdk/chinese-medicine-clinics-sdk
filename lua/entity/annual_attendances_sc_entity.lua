-- ChineseMedicineClinics SDK AnnualAttendancesSc entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AnnualAttendancesScEntity = {}
AnnualAttendancesScEntity.__index = AnnualAttendancesScEntity


function AnnualAttendancesScEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AnnualAttendancesScEntity)
  self._name = "annual_attendances_sc"
  self._client = client
...
function AnnualAttendancesScEntity:get_name()
  return self._name
end


function AnnualAttendancesScEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AnnualAttendancesScEntity.new(self._client, opts)
end
...
function AnnualAttendancesScEntity:mark_deleted()
  self._deleted = true
end


function AnnualAttendancesScEntity:deleted()
  return true == self._deleted
end


function AnnualAttendancesScEntity:data_set(args)
  if args ~= nil then
...
function AnnualAttendancesScEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AnnualAttendancesScEntity:match_set(args)
  if args ~= nil then
...
function AnnualAttendancesScEntity:match_get()
...
function AnnualAttendancesScEntity:stream(action, args, callopts)
...
---@param reqmatch AnnualAttendancesScListMatch
---@param ctrl? table
---@return AnnualAttendancesSc[]
---@return string? err
function AnnualAttendancesScEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function AnnualAttendancesScEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AnnualAttendancesScEntity
