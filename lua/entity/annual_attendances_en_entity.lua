...
local AnnualAttendancesEnEntity = {}
AnnualAttendancesEnEntity.__index = AnnualAttendancesEnEntity


function AnnualAttendancesEnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AnnualAttendancesEnEntity)
  self._name = "annual_attendances_en"
  self._client = client
...
function AnnualAttendancesEnEntity:get_name()
  return self._name
end


function AnnualAttendancesEnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AnnualAttendancesEnEntity.new(self._client, opts)
end
...
function AnnualAttendancesEnEntity:mark_deleted()
  self._deleted = true
end


function AnnualAttendancesEnEntity:deleted()
  return true == self._deleted
end


function AnnualAttendancesEnEntity:data_set(args)
  if args ~= nil then
...
function AnnualAttendancesEnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AnnualAttendancesEnEntity:match_set(args)
  if args ~= nil then
...
function AnnualAttendancesEnEntity:match_get()
...
function AnnualAttendancesEnEntity:stream(action, args, callopts)
...
function AnnualAttendancesEnEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return AnnualAttendancesEnEntity
