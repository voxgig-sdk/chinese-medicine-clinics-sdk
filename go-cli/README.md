...
boru-driven command-line client **and** interactive REPL for the ChineseMedicineClinics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chinese-medicine-clinics-cli)
make build

# 2. See usage (words, entities, env vars)
./chinese-medicine-clinics-cli --help
...
./chinese-medicine-clinics-cli list annual_attendances_sc

# 5. Override the API base URL for a single call
CHINESE_MEDICINE_CLINICS_BASE=https://api.example.com ./chinese-medicine-clinics-cli list annual_attendances_en

# 6. No arguments -> interactive REPL
./chinese-medicine-clinics-cli
chinese-medicine-clinics> list annual_attendances_en
chinese-medicine-clinics> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chinese-medicine-clinics-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/chinese-medicine-clinics-cli list annual_attendances_en
   ```
...
Run with no arguments to open a REPL (prompt `chinese-medicine-clinics>`). Each line is
evaluated as its own boru expression:

```text
$ ./chinese-medicine-clinics-cli
chinese-medicine-clinics> list annual_attendances_en
chinese-medicine-clinics> /help
chinese-medicine-clinics> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `annual_attendances_en`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/chinese-medicine-clinics-cli`. |
...
(`./chinese-medicine-clinics-cli <expr>`) and interactively (the REPL), and expressions compose the
