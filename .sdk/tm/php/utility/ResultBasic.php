...
// ChineseMedicineClinics SDK utility: result_basic

class ChineseMedicineClinicsResultBasic
{
    public static function call(ChineseMedicineClinicsContext $ctx): ?ChineseMedicineClinicsResult
    {
        ...
        if ($result && $response) {
            ...
            if ($result->status >= 400) {
                ...
                if ($result->err) {
                    $prev = ($result->err instanceof ChineseMedicineClinicsError) ? $result->err->msg : (string)$result->err;
