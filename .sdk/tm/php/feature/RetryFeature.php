...
// ChineseMedicineClinics SDK retry feature
...
class ChineseMedicineClinicsRetryFeature extends ChineseMedicineClinicsBaseFeature
{
    ...
    public function init(ChineseMedicineClinicsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ChineseMedicineClinicsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ChineseMedicineClinicsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ChineseMedicineClinicsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
