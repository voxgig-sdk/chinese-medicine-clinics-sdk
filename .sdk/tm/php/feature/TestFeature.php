...
// ChineseMedicineClinics SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class ChineseMedicineClinicsTestFeature extends ChineseMedicineClinicsBaseFeature
{
    ...
    public function init(ChineseMedicineClinicsContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (ChineseMedicineClinicsContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = ChineseMedicineClinicsParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (ChineseMedicineClinicsContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(ChineseMedicineClinicsContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = ChineseMedicineClinicsParam::call($ctx, $k);
