...
//    only - a typed ChineseMedicineClinicsContext there would make every ctx
...
final class ChineseMedicineClinicsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new ChineseMedicineClinicsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new ChineseMedicineClinicsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new ChineseMedicineClinicsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new ChineseMedicineClinicsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new ChineseMedicineClinicsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?ChineseMedicineClinicsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ChineseMedicineClinicsError(is_string($code) ? $code : '', $msg);
    }
