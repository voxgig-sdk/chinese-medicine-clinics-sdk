...
import (
	...
	omni "github.com/voxgig-sdk/chinese-medicine-clinics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ChineseMedicineClinicsError); is {
				out := map[string]any{
					"name":    "ChineseMedicineClinicsError",
...
func omniCtx(arg any, client *sdk.ChineseMedicineClinicsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ChineseMedicineClinicsSDK); is {
			client = live
