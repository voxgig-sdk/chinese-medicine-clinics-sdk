...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CHINESEMEDICINECLINICS_TEST_LIVE") == "TRUE" ||
		os.Getenv("CHINESEMEDICINECLINICS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CHINESEMEDICINECLINICS_TEST_EXPLAIN"); explain != "" {
