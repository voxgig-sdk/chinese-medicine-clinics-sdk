# ChineseMedicineClinics PHP SDK



The PHP SDK for the ChineseMedicineClinics API — an entity-oriented client using PHP conventions.
...
try {
    ...
    foreach ($annualattendancesens as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $annualattendancesen));
```
...
```
php/
├── chinesemedicineclinics_sdk.php          -- Main SDK class
...
The main class (`chinesemedicineclinics_sdk.php`) exports the SDK class
