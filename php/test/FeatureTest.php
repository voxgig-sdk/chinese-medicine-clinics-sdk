...
// ChineseMedicineClinics SDK feature test
//
...
class FtCtrl extends ChineseMedicineClinicsControl
{
...
class FtClient
{
    ...
    private ChineseMedicineClinicsUtility $utility;

    public function __construct(ChineseMedicineClinicsUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChineseMedicineClinicsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChineseMedicineClinicsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ChineseMedicineClinicsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ChineseMedicineClinicsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChineseMedicineClinicsConfig::make_feature($name);
    ...
    private static function buildUrl(ChineseMedicineClinicsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChineseMedicineClinicsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChineseMedicineClinicsResponse($fetched);
        }
    ...
    private function fail(ChineseMedicineClinicsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChineseMedicineClinicsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
