# ChineseMedicineClinics SDK test runner
...
class ChineseMedicineClinicsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ChineseMedicineClinicsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ChineseMedicineClinicsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ChineseMedicineClinicsTestRunner.getenv("CHINESE_MEDICINE_CLINICS_TEST_LIVE")
        override = ChineseMedicineClinicsTestRunner.getenv("CHINESE_MEDICINE_CLINICS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ChineseMedicineClinicsTestRunner.getenv(key)
        ...
        explain = ChineseMedicineClinicsTestRunner.getenv("CHINESE_MEDICINE_CLINICS_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ChineseMedicineClinicsTestRunner._test_control is not None:
            return ChineseMedicineClinicsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ChineseMedicineClinicsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ChineseMedicineClinicsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ChineseMedicineClinicsTestRunner.load_test_control()
...
def load_env_local():
    ChineseMedicineClinicsTestRunner.load_env_local()


def env_override(m):
    return ChineseMedicineClinicsTestRunner.env_override(m)


def entity_data(v):
    return ChineseMedicineClinicsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ChineseMedicineClinicsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ChineseMedicineClinicsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ChineseMedicineClinicsTestRunner.load_test_control()


def live_client_options():
    return ChineseMedicineClinicsTestRunner.live_client_options()


def live_delay_ms():
    return ChineseMedicineClinicsTestRunner.live_delay_ms()
