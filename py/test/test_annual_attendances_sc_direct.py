...
class TestAnnualAttendancesScDirect:

    def test_should_direct_list_annual_attendances_sc(self):
        setup = _annual_attendances_sc_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-annual_attendances_sc", "live" if setup["live"] else "unit")
        if _skip:
...
def _annual_attendances_sc_direct_setup(mockres):
