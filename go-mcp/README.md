# chinese-medicine-clinics-mcp

[MCP](https://modelcontextprotocol.io) server exposing the ChineseMedicineClinics SDK as
two agent tools — `chinese-medicine-clinics_list` and `chinese-medicine-clinics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chinese-medicine-clinics-mcp)
make build

# 2. Provide credentials via the environment
export CHINESE_MEDICINE_CLINICS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user chinese-medicine-clinics \
  -- /absolute/path/to/chinese-medicine-clinics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./chinese-medicine-clinics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// chinese-medicine-clinics_list: first page of records
{ "entity": "annual_attendances_en" }
{ "entity": "annual_attendances_en", "query": { } }

// chinese-medicine-clinics_load: one record by id
{ "entity": "annual_attendances_en", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chinese-medicine-clinics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user chinese-medicine-clinics \
     -- "$PWD"/dist/*/chinese-medicine-clinics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `chinese-medicine-clinics_list` and `chinese-medicine-clinics_load` tools now appear
   in new sessions. Ask the agent to *"list annual_attendances_en using chinese-medicine-clinics"*
   and it calls `chinese-medicine-clinics_list` with `{"entity":"annual_attendances_en"}`.
...
```sh
export CHINESE_MEDICINE_CLINICS_APIKEY=sk_live_xxx            # API key
export CHINESE_MEDICINE_CLINICS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `chinese-medicine-clinics_list` tool
...
### Call the `chinese-medicine-clinics_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `chinese-medicine-clinics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `chinese-medicine-clinics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHINESE_MEDICINE_CLINICS_APIKEY` | API key sent with every request. |
| `CHINESE_MEDICINE_CLINICS_BASE` | Optional override of the API base URL. |
...
```sh
./chinese-medicine-clinics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"chinese-medicine-clinics_load","arguments":{"entity":"annual_attendances_en","query":{"id":1}}}}'
```
