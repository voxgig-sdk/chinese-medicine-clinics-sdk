# ChineseMedicineClinics Ruby SDK



The Ruby SDK for the ChineseMedicineClinics API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.AnnualAttendancesEn` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = ChineseMedicineClinicsSDK.test(testopts, sdkopts)
```
...
raise a `ChineseMedicineClinicsError` (a `StandardError` subclass), so wrap
...
| --- | --- | --- |
| `attendances` | `Integer` | Number of attendances for the specified year |
...
```
rb/
├── ChineseMedicineClinics_sdk.rb       -- Main SDK module
...
The main module (`ChineseMedicineClinics_sdk`) exports the SDK class
...
annualattendancesen.list()

# annualattendancesen.data_get now returns the annualattendancesen data from the last list
# annualattendancesen.match_get returns the last match criteria
```
