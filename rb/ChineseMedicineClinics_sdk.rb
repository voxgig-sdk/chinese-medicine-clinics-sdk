...
require_relative 'ChineseMedicineClinics_types'
...
class ChineseMedicineClinicsSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ChineseMedicineClinicsHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ChineseMedicineClinicsFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ChineseMedicineClinicsUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ChineseMedicineClinicsError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue ChineseMedicineClinicsError => err
    ...
    if fetched.is_a?(Hash)
      status = ChineseMedicineClinicsHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ChineseMedicineClinicsError.new(
        "graphql_error", "ChineseMedicineClinicsSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ChineseMedicineClinicsSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
