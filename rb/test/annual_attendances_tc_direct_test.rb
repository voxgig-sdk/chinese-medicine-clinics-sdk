# AnnualAttendancesTc direct test
...
class AnnualAttendancesTcDirectTest < Minitest::Test
  def test_direct_list_annual_attendances_tc
    setup = annual_attendances_tc_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-annual_attendances_tc", setup[:live] ? "live" : "unit")
    if _should_skip
...
def annual_attendances_tc_direct_setup(mockres)
