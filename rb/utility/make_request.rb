...
module ChineseMedicineClinicsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ChineseMedicineClinicsResponse.new({})
    ...
    if fetch_err
      ...
      response = ChineseMedicineClinicsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ChineseMedicineClinicsResponse.new(fetched)
    else
