...
class AnnualAttendancesEnEntity
  ...
  def make
    opts = @_entopts.dup
    AnnualAttendancesEnEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [AnnualAttendancesEn, Hash] the current AnnualAttendancesEn data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ChineseMedicineClinicsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of AnnualAttendancesEn fields)
  def match_get
  ...
  #
  # @param reqmatch [AnnualAttendancesEnListMatch, Hash, nil] match filter (any subset of
  #   AnnualAttendancesEn fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<AnnualAttendancesEn>, Array] the matching AnnualAttendancesEn items; raises ChineseMedicineClinicsError on failure
